...
namespace rime
{
                                                   ...
                                                   TagMatching(ticket)
    {
        ...
        if (Config *config = engine_->schema()->config())
        {
            ...
            {
                tips_level_ = (tips == "all") ? kTipsAll : (tips == "char") ? kTipsChar
                                                                            : kTipsNone;
            }
    ...
    void Simplifier::Initialize()
    {
        ...
        rime::Path opencc_config_path = opencc_config_;
        // if (opencc_config_path.extension().string() == ".ini")
        if (opencc_config_path.extension() == ".ini")
        {
        ...
        if (opencc_config_path.is_relative())
        {
            ...
            rime::Path user_config_path = Service::instance().deployer().user_data_dir;
            rime::Path shared_config_path = Service::instance().deployer().shared_data_dir;
            (user_config_path /= "opencc") /= opencc_config_path;
            (shared_config_path /= "opencc") /= opencc_config_path;
            if (rime::Exists(user_config_path))
            {
                opencc_config_path = user_config_path;
            }
            else if (rime::Exists(shared_config_path))
            {
