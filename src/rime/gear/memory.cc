...
namespace rime
{
    ...
    Memory::Memory(const Ticket &ticket)
    {
        ...
        language_.reset(new Language{
            user_dict_ ? user_dict_->name() : Language::get_language_component(dict_->name())});

        Context *ctx = ticket.engine->context();
        commit_connection_ = connect(ctx, CommitNotifier, [this](Context *ctx)
                                     { OnCommit(ctx); });
        delete_connection_ = connect(ctx, DeleteNotifier, [this](Context *ctx)
                                     { OnDeleteEntry(ctx); });
        unhandled_key_connection_ = connect(ctx, UnhandledKeyEventNotifier, [this](Context *ctx, const KeyEvent &key)
