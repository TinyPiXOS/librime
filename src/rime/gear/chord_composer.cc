...
namespace rime
{
    ...
    ChordComposer::ChordComposer(const Ticket &ticket) : Processor(ticket)
    {
        ...
        update_connection_ = connect(ctx, UpdateNotifier, [this](Context *ctx)
                                     { OnContextUpdate(ctx); });
    ...
    {
        Context *ctx = engine_->context();
        disconnect(ctx, UpdateNotifier, update_connection_);
        disconnect(ctx, UnhandledKeyEventNotifier, unhandled_key_connection_);
