...
namespace rime
{
    ...
    an<Candidate> TableTranslation::Peek()
    {
        ...
        auto type = incomplete ? "completion" : is_user_phrase ? "user_table"
                                                               : "table";
                                           ...
                                           const Segment &segment)
    {
        ...
        rime::trim_right_if(code, rime::is_any_of(delimiters_));
    ...
    {
        return !contextual_suggestions_ ? string() : start > 0 ? engine_->context()->composition().GetTextBefore(start)
        ...
        for (size_t start_pos = 0; start_pos < input.length(); ++start_pos)
        {
            ...
            if (user_dict_ && user_dict_->loaded())
            {
                for (size_t len = 1; len <= active_input.length(); ++len)
                {
                    ...
                    if (resume_key > active_key && !rime::starts_with(resume_key, active_key))
                        break;
                }
            }
            if (encoder_ && encoder_->loaded())
            ...
            if (dict_ && dict_->loaded())
            {
                ...
                for (const auto &m : rime::adaptors::reverse(matches))
                {
                    if (m.length == 0)
                        continue;
                    size_t consumed_length =
