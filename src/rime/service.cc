...
namespace rime
{
    ...
    Session::Session()
    {
        ...
        connect(engine_, CommitSink, this, &Session::OnCommit);
        ...
        connect(engine_, MessageSink, [=](const string &message_type, const string &message_value)
                { Service::instance().Notify(session_id, message_type, message_value); });
    }
    ...
    Service::Service()
    {
        connect(&deployer_, MessageSink, [=](const string &message_type, const string &message_value)
                { Notify(0, message_type, message_value); });
