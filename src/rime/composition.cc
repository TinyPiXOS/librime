...
namespace rime
{
    ...
    string Composition::GetScriptText() const
    {
        ...
        for (const Segment &seg : *this)
        {
            ...
            if (cand && !cand->preedit().empty())
            {
                // 替换 boost::erase_first_copy
                std::string preedit = cand->preedit(); // 复制原字符串
                size_t pos = preedit.find('\t');       // 查找制表符
                if (pos != std::string::npos)
                {
                    preedit.erase(pos, 1); // 删除首个制表符
                }
                result += preedit; // 添加处理后的字符串
            }
            else
            {
    ...
    string Composition::GetTextBefore(size_t pos) const
    {
        ...
        for (const auto &seg : rime::adaptors::reverse(*this))
        {
            if (seg.end <= pos)
            {
                if (auto cand = seg.GetSelectedCandidate())
