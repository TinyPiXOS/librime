...
namespace rime
{
    ...
    {
        // return boost::filesystem::exists(prism_->file_name()) && boost::filesystem::exists(table_->file_name());
        return rime::Exists(prism_->file_name()) && rime::Exists(table_->file_name());
    }
    ...
    {
        // std::cout << "Dictionary *DictionaryComponent::Create(const Ticket &ticket) ++++"<< std::endl;
        if (!ticket.schema)
            return nullptr;
