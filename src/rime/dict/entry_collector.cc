...
namespace rime
{
    ...
    void EntryCollector::Collect(const string &dict_file)
    {
        ...
        while (getline(fin, line))
        {
            ...
            rime::split(row, line, rime::is_any_of("\t"));
                                     ...
                                     const string &weight_str)
    {
        ...
        bool scaled = rime::ends_with(weight_str, "%");
        ...
        if (scaled)
        {
            ...
            try
            {
                ...
                percentage = stod(weight_str.substr(0, weight_str.length() - 1));
            }
            catch (...)
            {
        ...
        else if (!weight_str.empty())
        { // absolute weight
            try
            {
                // e.weight = boost::lexical_cast<double>(weight_str);
                e.weight = stod(weight_str);
            }
            catch (...)
            {
                LOG(WARNING) << "invalid entry definition at #" << num_entries << ".";
