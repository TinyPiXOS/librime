...
namespace rime
{
    ...
    struct DfsState
    {
        ...
        bool IsExactMatch(const string &prefix)
        {
            ...
            return rime::starts_with(key, prefix + '\t');
        }
                                       ...
                                       string *resume_key)
    {
        ...
        while (accessor->GetNextRecord(&key, &value))
        {
            ...
            rime::trim_right(full_code); // remove trailing space a user dict key has
    ...
    {
        // std::cout << "Dictionary *ReverseLookupDictionaryComponent::Create(const Ticket &ticket) ++++"<< std::endl;
