...
namespace rime
{
    ...
    bool UserDbRecoveryTask::Run(Deployer *deployer)
    {
        ...
        if (db_->Exists())
        {
            ...
            std::error_code ec;
            rename_file(db_->file_name(), db_->file_name() + ".old", ec);
            if (ec && !db_->Remove())
            {
    ...
    void UserDbRecoveryTask::RestoreUserDataFromSnapshot(Deployer *deployer)
    {
        ...
        rime::erase_last(dict_name, component->extension());
        // locate snapshot file
        // boost::filesystem::path dir(deployer->user_data_sync_dir());
        rime::Path dir(deployer->user_data_sync_dir());
        // try *.userdb.txt
        // boost::filesystem::path snapshot_path = dir / (dict_name + component->snapshot_extension());
        rime::Path snapshot_path = dir / (dict_name + component->snapshot_extension());
        // if (!boost::filesystem::exists(snapshot_path))
        if (!rime::Exists(snapshot_path))
        {
            // try *.userdb.*.snapshot
    ...
    UserDbRecoveryTask *UserDbRecoveryTaskComponent::Create(TaskInitializer arg)
    {
        try
        {
            // 使用 std::any 替代 boost::any
            auto db = rime::any_cast<an<Db>>(arg);
