...
namespace rime
{
    ...
    ConcreteEngine::ConcreteEngine()
    {
        ...
        // 使用tinyPiXOS的信号槽
        connect(context_, CommitNotifier, [this](Context *ctx)
                { OnCommit(ctx); });
        connect(context_, SelectNotifier, [this](Context *ctx)
                { OnSelect(ctx); });
        connect(context_, UpdateNotifier, [this](Context *ctx)
                { OnContextUpdate(ctx); });

        connect(context_, OptionUpdateNotifier, [this](Context *ctx, const string &option)
                { OnOptionUpdate(ctx, option); });
        connect(context_, PropertyUpdateNotifier, [this](Context *ctx, const string &property)
    ...
    bool ConcreteEngine::ProcessKey(const KeyEvent &key_event)
    {
        ...
        context_->UnhandledKeyEventNotifier.emit(context_.get(), key_event);
        return false;
    }
    ...
    void ConcreteEngine::ApplySchema(Schema *schema)
    {
        ...
        MessageSink.emit("schema", schema->schema_id() + "/" + schema->schema_name());
    }
