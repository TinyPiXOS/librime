...
namespace rime
{
    ...
    string ConfigDependencyGraph::current_resource_id() const
    {
        ...
        std::string resource_id = key_stack.front();

        // 从右侧移除所有冒号
        auto it = std::find_if(resource_id.rbegin(), resource_id.rend(),
                               [](char c)
                               { return c != ':'; });

        resource_id.erase(it.base(), resource_id.end());
    ...
    inline static bool IsAppending(const string &key)
    {
        return key == ConfigCompiler::APPEND_DIRECTIVE ||
               ends_with(key, ADD_SUFFIX_OPERATOR);
    }
                                 ...
                                 bool merge_tree)
    {
        return key == ConfigCompiler::MERGE_DIRECTIVE ||
               ...
               (merge_tree && (!value || Is<ConfigMap>(value)) &&
                !ends_with(key, EQU_SUFFIX_OPERATOR));
    }

    inline static string StripOperator(const string &key, bool adding)
    {
        // 检查是否为指令
        if (key == ConfigCompiler::APPEND_DIRECTIVE || key == ConfigCompiler::MERGE_DIRECTIVE)
        {
            return "";
        }

        // 获取要移除的后缀
        const std::string &suffix = adding ? ADD_SUFFIX_OPERATOR : EQU_SUFFIX_OPERATOR;
        ...
        // 如果找到后缀且后缀在字符串末尾
        if (pos != std::string::npos && pos == key.size() - suffix.size())
        {
    ...
    Reference ConfigCompiler::CreateReference(const string &qualified_path)
    {
        ...
        string resource_id = resource_resolver_->ToResourceId(
            (separator == string::npos || separator == 0) ? graph_->current_resource_id() : qualified_path.substr(0, separator));
                                        ...
                                        const string &path)
    {
        for (const auto &x : graph->resolve_chain)
        {
            ...
            if (starts_with(x, path) && (x.length() == path.length() || x[path.length()] == '/'))
                return true;
        }
        return false;
    }
