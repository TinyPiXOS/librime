...
namespace rime
{
                                          ...
                                          UserDb::Component *component)
    {
        ...
        if (!rime::Exists(path_) || !rime::Path(path_).is_directory())
        {
            LOG(INFO) << "directory '" << path_.string() << "' does not exist.";
            return;
        }
        // for (boost::filesystem::directory_iterator it(path_), end; it != end; ++it)
        for (rime::Path::directory_iterator it(path_), end; it != end; ++it)
        {
            // string name = it->path().filename().string();
            string name = it.path().filename();
            // if (boost::ends_with(name, component->extension()))
            if (rime::ends_with(name, component->extension()))
            {
                // boost::erase_last(name, component->extension());
                rime::erase_last(name, component->extension());
    ...
    bool UserDictManager::Backup(const string &dict_name)
    {
        ...
        rime::Path dir(deployer_->user_data_sync_dir());
        if (!rime::Exists(dir))
        {
    ...
    bool UserDictManager::UpgradeUserDict(const string &dict_name)
    {
        ...
        rime::Path trash = rime::Path(deployer_->user_data_dir) / "trash";
        if (!rime::Exists(trash))
        {
            // boost::system::error_code ec;
            if (!rime::create_directories(trash))
            {
        ...
        string snapshot_file = dict_name + user_db_component_->snapshot_extension();
        rime::Path snapshot_path = trash / snapshot_file;
    ...
    bool UserDictManager::Synchronize(const string &dict_name)
    {
        ...
        bool success = true;
        rime::Path sync_dir(deployer_->sync_dir);
        if (!rime::Exists(sync_dir))
        {
            // boost::system::error_code ec;
            if (!rime::create_directories(sync_dir))
            {
        ...
        // *.userdb.txt
        string snapshot_file = dict_name + user_db_component_->snapshot_extension();
        // for (boost::filesystem::directory_iterator it(sync_dir), end; it != end; ++it)
        for (rime::Path::directory_iterator it(sync_dir), end; it != end; ++it)
        {
            if (!rime::Path(it.path()).is_directory())
                continue;
            rime::Path file_path = it.path() / snapshot_file;
            if (rime::Exists(file_path))
            {
