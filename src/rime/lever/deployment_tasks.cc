...
namespace rime
{
    ...
    DetectModifications::DetectModifications(TaskInitializer arg)
    {
        try
        {
            ...
            data_dirs_ = rime::any_cast<vector<string>>(arg);
        }
    ...
    bool DetectModifications::Run(Deployer *deployer)
    {
        ...
        try
        {
            ...
            {
                rime::Path p = rime::Path(dir).canonical();
                last_modified = (std::max)(last_modified, rime::last_write_time(p));
                // if (rime::Path::is_directory(p))
                if (p.is_directory())
                {
                    // for (boost::filesystem::directory_iterator iter(p), end; iter != end; ++iter)
                    for (rime::Path::directory_iterator iter(p), end; iter != end; ++iter)
                    {
                        rime::Path entry(iter.path());
                        // if (boost::filesystem::is_regular_file(boost::filesystem::canonical(entry)) &&
                        //     entry.extension().string() == ".yaml" &&
                        //     entry.filename().string() != "user.yaml")
                        if (rime::is_regular_file(rime::Path(entry).canonical()) &&
                            entry.extension() == ".yaml" &&
                            entry.filename() != "user.yaml")
                        {
                            // last_modified = (std::max)(last_modified, boost::filesystem::last_write_time(entry));
                            last_modified = (std::max)(last_modified, rime::last_write_time(entry));
                        }
                    }
                }
            }
    ...
    bool InstallationUpdate::Run(Deployer *deployer)
    {
        ...
        rime::Path shared_data_path(deployer->shared_data_dir);
        rime::Path user_data_path(deployer->user_data_dir);
        if (!rime::Exists(user_data_path))
        {
            LOG(INFO) << "creating user data dir: " << user_data_path.string();
            // boost::system::error_code ec;
            if (!rime::create_directories(user_data_path))
            {
                LOG(ERROR) << "Error creating user data dir: " << user_data_path.string();
            }
        }
        rime::Path installation_info(user_data_path / "installation.yaml");
        Config config;
        ...
        if (installation_id.empty())
        {
            ...
            installation_id = rime::uuids::random_generator()().to_string();
    ...
    bool WorkspaceUpdate::Run(Deployer *deployer)
    {
        ...
        auto build_schema = [&](const string &schema_id, bool as_dependency = false)
        {
            ...
            if (schema_path.empty() || !rime::Exists(schema_path))
            {
                if (as_dependency)
                {
    ...
    SchemaUpdate::SchemaUpdate(TaskInitializer arg) : verbose_(false)
    {
        try
        {
            ...
            schema_file_ = rime::any_cast<string>(arg);
        }
        // catch (const boost::bad_any_cast &)
        catch (...)
        {
    ...
    static bool MaybeCreateDirectory(rime::Path dir)
    {
        if (!rime::Exists(dir))
        {
            // boost::system::error_code ec;
            if (!rime::create_directories(dir))
            {
    ...
    static bool TrashDeprecatedUserCopy(const rime::Path &shared_copy,
                                        const rime::Path &user_copy,
                                        const string &version_key,
                                        const rime::Path &trash)
    {
        if (!rime::Exists(shared_copy) ||
            !rime::Exists(user_copy) ||
            rime::equivalent(shared_copy, user_copy))
        {
            return false;
        }
        ...
        if (cmp > 0 || (cmp == 0 && is_customized_user_copy))
        {
            ...
            rime::Path backup = trash / user_copy.filename();
            // boost::system::error_code ec;
            // boost::filesystem::rename(user_copy, backup, ec);
            std::error_code ec;
            // rime::rename(user_copy, backup, ec);
            // if (ec)
            if (!rime::rename(user_copy, backup, ec))
            {
    ...
    {
        rime::Path source_path(schema_file_);
        if (!rime::Exists(source_path))
        {
    ...
    ConfigFileUpdate::ConfigFileUpdate(TaskInitializer arg)
    {
        try
        {
            ...
            auto p = rime::any_cast<pair<string, string>>(arg);
            file_name_ = p.first;
    ...
    static bool ConfigNeedsUpdate(Config *config)
    {
        ...
        for (auto entry : *timestamps.AsMap())
        {
            ...
            rime::Path source_file = resolver->ResolvePath(entry.first);
            if (!rime::Exists(source_file))
            {
                if (recorded_time)
                {
            ...
            if (recorded_time != (int)rime::last_write_time(source_file))
            {
    ...
    {
        // std::cout << "*************deployer->shared_data_dir  " << deployer->shared_data_dir << std::endl;

        rime::Path shared_data_path(deployer->shared_data_dir);
        rime::Path user_data_path(deployer->user_data_dir);

        // std::cout << "************* shared_data_path*** " << shared_data_path << std::endl;
        // std::cout << "************* user_data_path*** " << user_data_path << std::endl;

        // trash depecated user copy created by an older version of Rime
        rime::Path source_config_path(shared_data_path / file_name_);
        rime::Path dest_config_path(user_data_path / file_name_);
        rime::Path trash = user_data_path / "trash";
    ...
    bool PrebuildAllSchemas::Run(Deployer *deployer)
    {
        rime::Path shared_data_path(deployer->shared_data_dir);
        rime::Path user_data_path(deployer->user_data_dir);
        if (!rime::Exists(shared_data_path) || !rime::Path(shared_data_path).is_directory())
            return false;
        bool success = true;
        // for (fs::directory_iterator iter(shared_data_path), end; iter != end; ++iter)
        for (rime::Path::directory_iterator iter(shared_data_path), end; iter != end; ++iter)
        {
            rime::Path entry(iter.path());
            if (rime::ends_with(entry.string(), ".schema.yaml"))
            {
    ...
    bool SymlinkingPrebuiltDictionaries::Run(Deployer *deployer)
    {
        ...
        rime::Path user_data_path(deployer->user_data_dir);
        if (!rime::Exists(shared_data_path) || !rime::Path(shared_data_path).is_directory() ||
            !rime::Exists(user_data_path) || !rime::Path(user_data_path).is_directory() ||
            rime::equivalent(shared_data_path, user_data_path))
            return false;
        bool success = false;
        // remove symlinks to shared data files created by previous version
        for (rime::Path::directory_iterator test(user_data_path), end; test != end; ++test)
        {
            rime::Path entry(test.path());
            if (rime::is_symlink(entry))
            {
                try
                {
                    // a symlink becomes dangling if the target file is no longer provided
                    // boost::system::error_code ec;
                    auto target_path = rime::Path(entry).canonical();
                    // bool bad_link = bool(ec);
                    bool linked_to_shared_data =
                        /*!bad_link &&*/
                        target_path.has_parent_path() &&
                        rime::equivalent(shared_data_path, target_path.parent_path());
                ...
                // catch (const fs::filesystem_error &ex)
                {
                    LOG(ERROR) << entry.string() << ": " /*<< ex.what()*/;
                    success = false;
                }
            }
        }
    ...
    {
        if (rime::ends_with(file_name, ".yaml") &&
            !rime::ends_with(file_name, ".custom.yaml"))
        {
            Config config;
            string checksum;
    ...
    bool BackupConfigFiles::Run(Deployer *deployer)
    {
        ...
        if (!rime::Exists(user_data_path))
            return false;
        rime::Path backup_dir(deployer->user_data_sync_dir());
        if (!MaybeCreateDirectory(backup_dir))
        {
            return false;
        }
        int success = 0, failure = 0, latest = 0, skipped = 0;
        for (rime::Path::directory_iterator iter(user_data_path), end; iter != end; ++iter)
        {
            rime::Path entry(iter.path());
            if (!rime::is_regular_file(entry))
                continue;
            auto file_extension = entry.extension();
            bool is_yaml_file = file_extension == ".yaml";
            bool is_text_file = file_extension == ".txt";
            if (!is_yaml_file && !is_text_file)
                continue;
            rime::Path backup = backup_dir / entry.filename();
            if (rime::Exists(backup) && Checksum(backup.string()) == Checksum(entry.string()))
            {
            ...
            bool ec = rime::copy_file(entry, backup, rime::copy_option::overwrite_if_exists);
            if (ec)
            {
                ++success;
            }
    ...
    bool CleanupTrash::Run(Deployer *deployer)
    {
        ...
        for (rime::Path::directory_iterator iter(user_data_path), end; iter != end; ++iter)
        {
            ...
            if (filename == "rime.log" ||
                rime::ends_with(filename, ".bin") ||
                rime::ends_with(filename, ".reverse.kct") ||
                rime::ends_with(filename, ".userdb.kct.old") ||
                rime::ends_with(filename, ".userdb.kct.snapshot"))
            {
                if (!success && !MaybeCreateDirectory(trash))
                {
                    return false;
                }
                rime::Path backup = trash / entry.filename();
                //  boost::system::error_code ec;
                std::error_code ec;
                if (rime::rename(entry, backup, ec))
                {
                    ++success;
                }
                else
    ...
    bool CleanOldLogFiles::Run(Deployer *deployer)
    {
        ...
        for (auto i = dirs.cbegin(); i != dirs.cend(); ++i)
        {
            ...
            for (rime::Path::directory_iterator j(*i), end; j != end; ++j)
            {
                rime::Path entry(j.path());
                string file_name(entry.filename());
                try
                {
                    if (rime::is_regular_file(entry) &&
                        !rime::is_symlink(entry) &&
                        rime::starts_with(file_name, "rime.") &&
                        !rime::contains(file_name, today))
                    {
                ...
                // catch (const fs::filesystem_error &ex)
                {
                    LOG(ERROR) << "deployment_tasks.cc 827 "/*ex.what()*/;
                    success = false;
                }
            }
        }
