...
namespace rime
{
    ...
    bool Customizer::UpdateConfigFile()
    {
        ...
        if (rime::Exists(source_path_) &&
            rime::Exists(dest_path_) &&
            rime::equivalent(source_path_, dest_path_))
        {
        ...
        rime::Path custom_path(dest_path_);
        ...
        if (!custom_path.empty() && rime::Exists(custom_path))
        {
        ...
        if (redistribute || (is_dirty && !missing_original_copy))
        {
            try
            {
                rime::copy_file(source_path_, dest_path_, rime::copy_option::overwrite_if_exists);
                // boost::filesystem::copy_file(source_path_, dest_path_, boost::filesystem::copy_option::overwrite_if_exists);
            }
            catch (...)
            {
