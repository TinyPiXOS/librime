...
namespace rime
{
    ...
    void SwitcherSettings::GetAvailableSchemasFromDirectory(const rime::Path &dir)
    {
        // if (!rime::Exists(dir) || !rime::Path::is_directory(dir))
        if (!rime::Exists(dir) || !rime::Path(dir).is_directory())
        {
            LOG(INFO) << "directory '" << dir.string() << "' does not exist.";
            return;
        }
        for (rime::Path::directory_iterator it(dir), end; it != end; ++it)
        {
            string file_path(it.path().string());
            if (rime::ends_with(file_path, ".schema.yaml"))
            {
                Config config;
